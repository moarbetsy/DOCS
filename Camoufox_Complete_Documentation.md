...
> Complete documentation extracted from [camoufox.com](https://camoufox.com)
...
##### Warning
This isn't recommended, as Camoufox will populate this data for you automatically. See the parameters list for more proper usage.
```python
from camoufox.sync_api import Camoufox
with Camoufox(
    config={
...
Nearly all commerical anti-detect solutions add noise to the canvas to avoid canvas fingerprinting, even on shapes that render the same on every device.
This technique is easily detectable by drawing a rectangle and comparing it to an expected result.
...
| --- | --- | --- | --- |
| `voices` | array | An array of maps for each voice. Each map must contain the keys `isLocalService`, `isDefault`, `voiceUri`, `name`, and `lang`. | ```python [   {     "isLocalService": true,     "isDefault": true,     "voiceUri": "Ting-Ting",     "name": "Ting-Ting",     "lang": "zh-CN"   },   ... ] ``` |
...
i
Compares pixel data between two canvases to detect modifications in color channels to find potential canvas fingerprinting countermeasures.
Result:
...
### Text Rendering
i
Renders text using different fonts and measures text metrics to find differences in font rendering across systems.
Hash:
Font Metrics:
...
i
Draws various shapes using canvas methods, testing how different systems render basic geometric forms.
Hash:
...
### Shadows
i
Applies shadow effects to a rectangle to test how different systems process and render shadow blur and color.
Hash:
...
### Text Shadow
i
Applies a shadow effect to text to find differences in text shadow processing across systems.
Hash:
...
### Gradients
i
Creates a linear gradient and fills the canvas, testing how different systems interpolate colors in gradients.
Hash:
...
i
Uses the 'multiply' composite operation to blend shapes, testing for differences in how systems process complex rendering operations.
Hash:
...
i
Combines all previous tests into a single canvas to create a hash representing the system's rendering characteristics.
...
# Click button page
This is a demo page for testing clicks within the shadow dom.
Click me (basic)
Shadow DOM Example
...
matchMedia.mediaCss.color-gamut --> system\_styles.colors
...
navigator.userAgent --> network.headers\_fp
network.headers\_fp --> network.tls.scrapfly\_fp
network.headers\_fp --> network.http2.fingerprint
network.http2.fingerprint --> network.http2.http2\_frames
network.headers\_fp --> headers.accept-encoding
network.headers\_fp --> headers.accept
network.headers\_fp --> headers.priority
network.headers\_fp --> headers.sec-ch-ua
network.headers\_fp --> headers.sec-ch-ua-mobile
network.headers\_fp --> headers.sec-ch-ua-platform
network.headers\_fp --> headers.sec-fetch-dest
network.headers\_fp --> headers.sec-fetch-mode
network.headers\_fp --> headers.sec-fetch-site
network.headers\_fp --> headers.sec-gpc
...
# Low-Entropy Canvas Test


## Extracted Data Grid


## Possible Data Grids (Sorted by RMSE)


## Offset Grid (Extracted - Best Candidate)
...
| Vendor | Renderer | Probability | Sufficient |
| --- | --- | --- | --- |
| Apple | Apple M1, or similar | 82.2% | ✅ |
| Intel Inc. | Intel(R) HD Graphics 400, or similar | 7.4% | ✅ |
| ATI Technologies Inc. | Radeon R9 200 Series, or similar | 3.7% | ✅ |
| Intel Inc. | Intel(R) HD Graphics, or similar | 3.3% | ✅ |
| Google Inc. (NVIDIA) | ANGLE (NVIDIA, NVIDIA GeForce GTX 980 Direct3D11 vs\_5\_0 ps\_5\_0), or similar | 0.4% | ✅ |
| Google Inc. (NVIDIA) | ANGLE (NVIDIA, NVIDIA GeForce GTX 980 Direct3D11 vs\_5\_0 ps\_5\_0), or similar | 0.4% | ✅ |
| NVIDIA Corporation | NVIDIA GeForce GTX 480 | 1.2% | ❌ |
| ATI Technologies Inc. | Radeon R9 200 Series | 0.4% | ❌ |
| Google Inc. | ANGLE (NVIDIA GeForce GTX 1070 Direct3D11 vs\_5\_0 ps\_5\_0) | 0.4% | ❌ |
| NVIDIA Corporation | NVIDIA GeForce GTX 980 | 0.4% | ❌ |
| Apple | Apple M1 | 0.4% | ❌ |
Linux
| Vendor | Renderer | Probability | Sufficient |
| --- | --- | --- | --- |
| Intel | Intel(R) HD Graphics, or similar | 27.5% | ✅ |
| NVIDIA Corporation | NVIDIA GeForce GTX 980, or similar | 25.0% | ✅ |
| Intel | Intel(R) HD Graphics 400, or similar | 15.0% | ✅ |
| AMD | Radeon R9 200 Series, or similar | 10.0% | ✅ |
| AMD | Radeon HD 3200 Graphics, or similar | 8.1% | ✅ |
| Mesa | llvmpipe, or similar | 5.6% | ✅ |
| Intel | Intel(R) HD Graphics 400 | 1.9% | ✅ |
| Mesa | llvmpipe | 1.9% | ✅ |
| Mesa/X.org | llvmpipe, or similar | 1.2% | ✅ |
| NVIDIA Corporation | NVIDIA GeForce GTX 980/PCIe/SSE2 | 0.6% | ✅ |
| Mesa | GeForce GTX 980, or similar | 0.6% | ✅ |
| VMware, Inc. | llvmpipe, or similar | 0.6% | ❌ |
| Mesa | Apple M1, or similar | 0.6% | ❌ |
| Mesa | GeForce GTX 480, or similar | 0.6% | ❌ |
| Intel | Intel(R) HD Graphics | 0.6% | ❌ |

--------------------------------------------------

# Source: https://camoufox.com/tests/canvas
...
# Source: https://camoufox.com/tests/shadowdom
...
# Source: https://camoufox.com/assets/fpgen/tree
...
# Source: https://camoufox.com/tests/private/canvas-low-entropy
